...
const MOCK_TASKS: Task[] = [
  {
    ...
    sample_count: { total: 45, threshold: 30 },
  ...
  },
  {
    ...
    sample_count: { total: 12, threshold: 30 },
...
const App: React.FC = () => {
  ...
  const handleCreateTask = (newTaskPartial: Omit<Task, 'id' | 'created_at' | 'status' | 'sample_count'>) => {
    const newTask: Task = {
      ...newTaskPartial,
      id: `task_${Date.now()}`,
      created_at: new Date().toISOString(),
      status: TaskStatus.INIT,
      sample_count: { total: 0, threshold: 30 }
    };
                           ...
                           <tr key={task.id} className="hover:bg-gray-50 transition-colors group">
                              ...
                              <td className="px-6 py-4">
                                 <div className="flex items-center text-sm">
                                    <span className="font-medium mr-2 text-gray-700">{task.sample_count.total}/{task.sample_count.threshold}</span>
                                    <div className="w-16 h-1.5 bg-gray-200 rounded-full">
                                       <div 
                                          className={`h-full rounded-full ${task.sample_count.total >= task.sample_count.threshold ? 'bg-green-500' : 'bg-yellow-400'}`} 
                                          style={{ width: `${Math.min(100, (task.sample_count.total/task.sample_count.threshold)*100)}%` }}
                                       ></div>
                                    </div>
                                 </div>
                              </td>
